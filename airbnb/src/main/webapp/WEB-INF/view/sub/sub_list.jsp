   ...
   <div class="menu2">
     ...
     <div	class="row">
     	...
     	<c:forEach var="i" begin="${start }" end="${end }" step="1">
     		<c:choose>
     			<c:when test="${i == page}">
     				<a href="${pageContext.request.contextPath}/sub/sub_list?page=${i}" class="this">${i}</a>
     			</c:when>
     			<c:otherwise>
		...
		<c:if test="${page < totalPage - 1 && end < totalPage}">
     		<span>&nbsp&nbsp···&nbsp </span>
     		<a href="${pageContext.request.contextPath }/sub/sub_list?page=${totalPage}">${totalPage}</a>
     	</c:if> 
     </div>
