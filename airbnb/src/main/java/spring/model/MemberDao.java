	...
	public boolean modify(Member m) {
		try {
			...
			if (m.getNo() != 0) {
				...
				if (m.getEmail() != null && !m.getEmail().isEmpty() && !orig.getEmail().equals(m.getEmail())) {
					return false; // 수정불가
				}
				if (m.getPw() != null && !m.getPw().isEmpty() && !encoder.matches(m.getPw(), orig.getPw())) {
					String sql = "update member set pw = ? where no = ?";
					if (jdbcTemplate.update(sql, encoder.encode(m.getPw()), m.getNo()) < 0)
						return false;
				}
				if (m.getName() != null && !m.getName().isEmpty() && !orig.getName().equals(m.getName())) {
					String sql = "update member set name = ? where no = ?";
					if (jdbcTemplate.update(sql, m.getName(), m.getNo()) < 0)
						return false;
				}
				if (m.getPhone() != null && !m.getPhone().isEmpty() && !orig.getPhone().equals(m.getPhone())) {
					String sql = "update member set phone = ? where no = ?";
					if (jdbcTemplate.update(sql, m.getPhone(), m.getNo()) < 0)
						return false;
				}
				if (m.getAuthority() != null && !m.getAuthority().isEmpty()
				...
				if (m.getReg() != null && !m.getReg().isEmpty() && !orig.getReg().equals(m.getReg())) {
					return false; // 수정불가
				}
				return true; // 수정완료
			}
